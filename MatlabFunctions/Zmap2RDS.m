...
if ~exist('RDSFile','var') || isempty(RDSFile)
...
% if any(Display.Rect([3,4])/2 ~= DisplayRes)
...
if ~isfield(Display,'win')
    ...
    [Display.win, Display.rect] = PsychImaging('OpenWindow', Display.ScreenID, 127, Display.Rect, [], [], Display.Stereomode);
...
%Tex = Screen('MakeTexture', Display.win,  repmat(BackgroundRGB(1), size(Display.Rect([3,4]))));
...
%     figure; image(RDScapture); axis equal tight;
%     Display.Rect
%     Display.rect
%     size(RDScapture)
%     RDScapture = imresize(RDScapture, [size(RDScapture,1), size(RDScapture,2)/2]);
    RDScapture = imresize(RDScapture, [Display.Rect(4), Display.Rect(3)/2]);
end
if SaveAlpha == 0
