function write_quadobj(filename, vertex, face, texcoord, options)
...
%   texcoord must be of size [n,2]
%
%   Copyright (c) 2004 Gabriel Peyr�
%   Updated to accept quads - 2016 Aidan Murphy
...
if isfield(options, 'nm_file')
    ...
    if exist('texcoord','var')
        if find(size(texcoord)==2) == 1
            texcoord = texcoord';
        end
        ...
        vertext = vertex(:,1:2)*0 - 1;  % create dummy vertex texture
        % vertex position
        fprintf(fid, '# %d vertex texture\n', size(vertext,1));
        fprintf(fid, 'vt %f %f\n', vertext');
    end
...
% face
fprintf(fid, '# %d faces\n', size(face,1));
face_texcorrd = [face(:,1), face(:,1), face(:,2), face(:,2), face(:,3), face(:,3), face(:,4), face(:,4)];
