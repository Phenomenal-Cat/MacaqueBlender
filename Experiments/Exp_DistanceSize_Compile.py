...
def GetAllParamsFiles():
    ...
    FramesDir   = os.path.join(Prefix, 'murphya/Stimuli/AvatarRenders_2018/SizeDistance/Frames/')
    OutputDir   = os.path.join(Prefix, 'murphya/Stimuli/AvatarRenders_2018/SizeDistance/Movies/')
    ...
    Expressions = ['Neutral']
    HazAngles   = [0]
    FileSuffix  = ['Scale0.67', 'Scale1.00', 'Scale1.33', 'Dist-20', 'Dist0','Dist20', 'ConstantRetinalSize', 'LateralMotion_Dist0_Scale1.00']
    NoFrames    = 60
    MovNo       = 0
    for ob in Objects:
        for exp in Expressions:
            for haz in HazAngles:
                for Suf in FileSuffix:
                    FrameImages.append([])
                    MovieName.append('%s_%s_Haz%d_' % (ob, exp, haz) + Suf)
